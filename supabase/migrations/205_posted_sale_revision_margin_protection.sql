...
declare
  v_user_id uuid;
  v_policy public.sales_margin_policy%rowtype;
  v_line record;
...
begin
  ...
  if not public.is_admin() then
    raise exception
      'You are not authorized to validate posted-sale revision margins.';
  end if;
  ...
  if not found then
    raise exception 'No active sales margin policy exists.';
  end if;
  ...
  for v_line in
    ...
    where sales_order_id = p_sales_order_id
      and fulfilment_method = 'stock'
    order by line_number
  loop

    if v_line.current_unit_cost is null
       or v_line.current_unit_cost <= 0
    then
      v_missing_cost_count := v_missing_cost_count + 1;

      if v_policy.block_when_cost_missing then
        raise exception
          'Posted-sale modification blocked. Sales Order % line % (%) has no valid inventory cost.',
          v_line.order_number,
          v_line.line_number,
          v_line.item_name;
      end if;
    ...
    if v_line.estimated_margin_percentage is not null then
      if v_lowest_margin is null
         or v_line.estimated_margin_percentage < v_lowest_margin
      then
        v_lowest_margin := v_line.estimated_margin_percentage;
      end if;
    end if;

    if v_line.estimated_margin_percentage
    ...
    then
      ...
      if v_policy.block_below_minimum then
        raise exception
          'Posted-sale modification blocked. Sales Order % line % (%) margin is %%%, below the minimum %%%. A new revision-specific approval is required for this commercial state.',
          v_line.order_number,
          v_line.line_number,
          v_line.item_name,
          round(v_line.estimated_margin_percentage,2),
          round(v_policy.minimum_margin_percentage,2);
      end if;

    elsif v_line.estimated_margin_percentage
  ...
  return jsonb_build_object(
    ...
    'warningMarginPercentage', v_policy.warning_margin_percentage,
    'minimumMarginPercentage', v_policy.minimum_margin_percentage,
    'warningLines', v_warning_count,
    'blockedLines', v_blocked_count,
    'missingCostLines', v_missing_cost_count,
    'lowestMarginPercentage', v_lowest_margin,
    'historicalOrderApprovalAccepted', false
  );
end;
$$;
