/*
 * HM ERP — Stock-First Local Purchase
 *
 * Local-purchase products consume existing warehouse stock first.
 * Only the remaining shortage requires procurement.
 *
 * Import-on-demand and dropship remain procurement-only.
 * Service lines remain non-inventory.
 */
...
begin
  ...
  /*
   * Draft planning values are informational only.
   * Reset them before creating real reservations.
   */
  ...
  loop
    ...
    /* =====================================================
     * Stock / Local Purchase
     *
     * Both methods consume existing warehouse stock first.
     * Local Purchase procures only the shortage.
     * ===================================================== */
    ...
    /*
     * A custom local-purchase line may not yet have
     * a catalog product. It cannot consume inventory.
     */
    ...
    /*
     * Local Purchase without a warehouse cannot consume
     * existing inventory, so procure the complete amount.
     */
    ...
    /*
     * Lock exact inventory record so concurrent orders
     * cannot reserve the same availability.
     */
    select *
    into v_stock
...
begin
  ...
  loop
    ...
    /*
     * Stock-first local-purchase lines can now hold real
     * warehouse reservations, so they must be released too.
     */
    if (
      v_item.fulfilment_method in (
        'stock',
        'local_purchase'
      )
