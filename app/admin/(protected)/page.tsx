...
import { getSalesOrderSummary } from "@/lib/repositories/sales-order.repository";
...
export default async function AdminDashboardPage() {
  ...
  const dateFrom =
    formatDateForRepository(monthStart);

    const sales =
    await getSalesOrderSummary();

  const trade =
    await getReceivablesPayablesDashboard();

  const financial =
    await getFinancialAccountSummary();
  ...
  const ordersToFulfil =
    sales.confirmed +
    sales.processing +
    sales.partiallyFulfilled;

  const procurementNeeded =
    sales.awaitingProcurement +
    sales.awaitingStock;
    ...
    <div className="mx-auto max-w-[1600px] space-y-8">
      <section className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Live overview of sales, collections, supplier
            obligations, cash, inventory and profitability.
          </p>
        </div>
      ...
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricCard
            ...
            value={money(
              trade.summary.totalReceivables,
            )}
            description={`${trade.summary.openReceivableCount} open receivable${
              trade.summary.openReceivableCount === 1
                ? ""
                : "s"
            }`}
            ...
            icon={Users}
            attention={
              trade.summary.overdueReceivables >
              0
            }
          />
          ...
          <MetricCard
            ...
            value={money(
              trade.summary.totalPayables,
            )}
            description={`${trade.summary.openPayableCount} open payable${
              trade.summary.openPayableCount === 1
                ? ""
                : "s"
            }`}
            href="/admin/accounts/receivables-payables"
      ...
      <section>
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Orders, procurement, stock and current-month
            profitability.
          </p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricCard
            title="Net Profit"
            value={money(
              profitability.summary.netProfit,
            )}
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-2xl font-bold">
              {money(
                trade.summary.netTradePosition,
              )}
            </p>

            <p className="mt-1 text-sm text-slate-400">
              Net receivable position after customer
              advances versus supplier exposure.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm sm:grid-cols-4">
            <MiniValue
              ...
              value={money(
                trade.summary.customerAdvances,
              )}
            />
            ...
            <MiniValue
              ...
              value={money(
                trade.summary.supplierAdvances,
              )}
            />
            ...
            <MiniValue
              ...
              value={money(
                trade.summary.collectionsLast30Days,
              )}
            />
...
function formatDateForRepository(
  value: Date,
): string {
  const year =
    value.getFullYear();
