...
export default function NewCustomerReceiptForm({
  ...
  const [paymentMethod, setPaymentMethod] =
    useState<CustomerReceiptPaymentMethod>("cash");
  ...
  const totalOutstanding = useMemo(
    () => orders.reduce((total, order) => total + order.balanceDue, 0),
    [orders],
  );
  ...
  const automaticAllocations = useMemo(() => {
    ...
    for (const order of orders) {
      ...
      const allocation = Math.min(order.balanceDue, remaining);

      if (allocation > 0) {
        result[order.id] = allocation;
  ...
  function handleCustomerChange(nextCustomerId: string) {
    ...
    startLoadingOrders(async () => {
      try {
        const result = await loadCustomerOutstandingOrders(nextCustomerId);
  ...
  function updateManualAllocation(
    order: CustomerOutstandingOrder,

    value: number,
  ) {
    const next = Math.max(Math.min(value, order.balanceDue), 0);
  ...
  function handleSubmit() {
    ...
    if (allocatedAmount > amount + 0.01) {
      toast.error("Allocated amount cannot exceed receipt amount.");
    ...
    startPosting(async () => {
      ...
      router.push("/admin/sales/receipts");
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
      <div className="space-y-6">
        ...
        <section className="rounded-xl border bg-card p-6">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <Field label="Receipt Amount" required>
              <input
                type="number"
                min={0}
                step="0.01"
            ...
            <Field label="Payment Method" required>
              <select
                ...
                onChange={(event) => {
                  setPaymentMethod(
                    event.target.value as CustomerReceiptPaymentMethod,
                  );
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  {orders.map((order) => {
                    const allocation = activeAllocations[order.id] ?? 0;
                    ...
                    return (
                      <tr key={order.id}>
                        ...
                        <td className="px-4 py-4 text-right">
                          AED {money(order.grandTotal)}
                        </td>

                        <td className="px-4 py-4 text-right">
                          AED {money(order.paidAmount)}
                        </td>

                        <td className="px-4 py-4 text-right font-semibold text-amber-700">
                          AED {money(order.balanceDue)}
                        </td>
      ...
      <aside className="h-fit rounded-xl bg-slate-950 p-6 text-white xl:sticky xl:top-24">
        ...
        <div className="mt-6 space-y-4 text-sm">
          <SummaryRow label="Customer Outstanding" value={totalOutstanding} />

          <div className="border-t border-slate-700 pt-4">
            <SummaryRow label="Receipt Amount" value={amount} strong />
          </div>
        ...
        {unallocatedAmount > 0 ? (
          <div className="mt-5 rounded-lg bg-amber-500/10 p-3 text-xs leading-5 text-amber-200">
            AED {money(unallocatedAmount)} will remain as unallocated customer
            credit.
          </div>
        ) : null}
        ...
        >
          ...
          {isPosting ? "Posting Receipt..." : "Post Receipt"}
        </button>
      </aside>
    </div>
  );
